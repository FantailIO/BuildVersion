 ...
 */
function saveVersionFile(options, versionInfo) {
 ...
 */
function decipherGitBranch(options) {
 ...
 */
function getBranchVersionConfig(options, versionInfo) {
  var index = decipherGitBranch(options);
  if(options.config) {
    config = readConfigFile(options);
  } else {
...
function updateVersionInfo(versionInfo, options) {
  ...
  var config = getBranchVersionConfig(options, versionInfo);
 ...
 */
function processCommandLine(testingOptions) {
  var exitCode = 0;
  try {
    var options = {};
...
if(options.dumpConfig) {
  ...
  if(!options.dryrun) {
    saveVersionFile(options, versionInfo);
  } else {
